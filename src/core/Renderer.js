...
export class Renderer {
    ...
    colorSky(speed) {
        const maxSkySaturation =
            +this.app.stateManager.state.config.skyLighting * 15;
        ...
        this.targetSkyColor.r =
            (this.targetSkyColor.r / maxColorComponent) *
            maxSkySaturation *
            intensity;
        this.targetSkyColor.g =
            (this.targetSkyColor.g / maxColorComponent) *
            maxSkySaturation *
            intensity;
        this.targetSkyColor.b =
            (this.targetSkyColor.b / maxColorComponent) *
            maxSkySaturation *
            intensity;
        const colorChange = 10;
        this.currentSkyColor.r +=
            ((this.targetSkyColor.r - this.currentSkyColor.r) / colorChange) *
            speed;
        this.currentSkyColor.g +=
            ((this.targetSkyColor.g - this.currentSkyColor.g) / colorChange) *
            speed;
        this.currentSkyColor.b +=
            ((this.targetSkyColor.b - this.currentSkyColor.b) / colorChange) *
            speed;
    ...
    render(speed) {
        ...
        const sf = this.app.stateManager.scaleFactor || 1;
        if (
            +this.app.stateManager.state.config.skyLighting !==
            Constants.SKY_LIGHT_NONE
        )
            this.colorSky(speed);
        ...
        trailsCtx.lineWidth =
            this.app.stateManager.quality === Constants.QUALITY_HIGH ? 0.75 : 1;
        trailsCtx.lineCap =
            this.app.stateManager.quality === Constants.QUALITY_LOW
                ? 'square'
                : 'round';
