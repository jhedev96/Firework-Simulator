...
export class SequenceManager {
    ...
    seqRandomShell() {
        ...
        const shell = new Shell(
            this.app.shellFactory.shellFromConfig(size),
            this.app,
        );
        ...
        return (
            900 +
            Math.random() * 600 +
            (shell.fallingLeaves ? 4600 : shell.starLife)
        );
    }
    ...
    seqRandomFastShell() {
        const { size, x, height } = this.getRandomShellSize();
        const shell = new Shell(
            ShellFactory.randomFastShell()(size, this.app),
            this.app,
        );
        shell.launch(x, height);
    ...
    seqTwoRandom() {
        ...
        const shell1 = new Shell(
            this.app.shellFactory.shellFromConfig(s1.size),
            this.app,
        );
        const shell2 = new Shell(
            this.app.shellFactory.shellFromConfig(s2.size),
            this.app,
        );
        shell1.launch(0.3 + Math.random() * 0.2 - 0.1, s1.height);
        setTimeout(
            () => shell2.launch(0.7 + Math.random() * 0.2 - 0.1, s2.height),
            100,
        );
        return (
            900 +
            Math.random() * 600 +
            Math.max(shell1.starLife, shell2.starLife) +
            (shell1.fallingLeaves || shell2.fallingLeaves ? 4600 : 0)
        );
    }
    ...
    seqTriple() {
        ...
        new Shell(shellType(baseSize, this.app), this.app).launch(
            0.5 + Math.random() * 0.08 - 0.04,
            0.7,
        );
        setTimeout(
            () =>
                new Shell(shellType(smallSize, this.app), this.app).launch(
                    0.2 + Math.random() * 0.08 - 0.04,
                    0.1,
                ),
    ...
    seqPyramid() {
        ...
        const randomMainShell =
            Math.random() < 0.78
                ? ShellFactory.shellTypes['Crysanthemum']
                : ShellFactory.shellTypes['Ring'];
        const launchShell = (x, useSpecial) => {
            const shellType =
                this.app.stateManager.shellName === 'Random'
                    ? useSpecial
                        ? ShellFactory.randomShell
                        : randomMainShell
                    : ShellFactory.shellTypes[this.app.stateManager.shellName];
            new Shell(
                shellType(useSpecial ? largeSize : smallSize, this.app),
                this.app,
            ).launch(
                x,
                useSpecial ? 0.75 : (x <= 0.5 ? x / 0.5 : (1 - x) / 0.5) * 0.42,
            );
        };
        let count = 0,
            delay = 0;
    ...
    seqSmallBarrage() {
        ...
        const launchShell = (x, useSpecial) => {
            const shellType =
                this.app.stateManager.shellName === 'Random'
                    ? useSpecial
                        ? ShellFactory.randomFastShell()
                        : randomMainShell
                    : ShellFactory.shellTypes[this.app.stateManager.shellName];
            new Shell(shellType(shellSize, this.app), this.app).launch(
                x,
                ((Math.cos(x * 5 * Math.PI + Constants.PI_HALF) + 1) / 2) *
                    0.75,
            );
        };
        let count = 0,
    ...
    startSequence() {
        if (this.isFirstSeq) {
            ...
            new Shell(
                ShellFactory.shellTypes['Crysanthemum'](
                    this.app.stateManager.shellSize,
                    this.app,
                ),
                this.app,
            ).launch(0.5, 0.5);
        ...
        if (rand < 0.08 && Date.now() - this.cooldowns.seqSmallBarrage > 15000)
