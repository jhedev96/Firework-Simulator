...
export class UIManager {
    ...
    setOptions(node, options) {
        ...
        node.innerHTML = options.reduce(
            (acc, opt) =>
                (acc += `<option value="${opt.value !== undefined ? opt.value : opt}">${opt.label || opt}</option>`),
            '',
        );
    ...
    updateSelectOptions() {
        ...
        this.setOptions(
            this.nodes.shellSize,
            ['3"', '4"', '6"', '8"', '12"', '16"'].map((opt, i) => ({
                label: opt,
                value: i,
            })),
        );
        ...
        this.setOptions(
            this.nodes.scaleFactor,
            [0.5, 0.62, 0.75, 0.9, 1.0, 1.5, 2.0].map((v) => ({
                value: v.toFixed(2),
                label: `${v * 100}%`,
            })),
        );
        ...
        if (this.nodes.appLanguage)
            this.nodes.appLanguage.value = this.i18n.selectedSetting;
    }
    ...
    initDOM() {
        if (
            ...
            this.nodes.fullscreenFormOption
        )
    ...
    bindEvents() {
        ...
        [
            ...
            if (this.nodes[key])
                this.nodes[key].addEventListener('input', updateConfig);
        });
        ...
        [
            ...
            if (this.nodes[key])
                this.nodes[key].addEventListener('click', () =>
            ...
            this.nodes.fullscreen.addEventListener('click', () => {
                setTimeout(
                    () =>
                        ...
                        this.app.inputManager.toggleFullscreen(),
                    0,
                );
            });
        }
        ...
        Object.entries(Constants.HELP_TOPIC_SELECTORS).forEach(
            ([selector, topicKey]) => {
    ...
    render(state) {
        ...
        this.nodes.pauseBtnSVG.setAttribute(
            'href',
            `#icon-${state.paused ? 'play' : 'pause'}`,
        );
        this.nodes.soundBtnSVG.setAttribute(
            'href',
            `#icon-sound-${state.soundEnabled ? 'on' : 'off'}`,
        );
        this.nodes.controls.classList.toggle(
            'hide',
        ...
        this.nodes.finaleModeFormOption.style.opacity = state.config.autoLaunch
            ? 1
            : 0.32;
        this.nodes.wordText.parentElement.style.opacity = state.config.wordShell
            ? 1
            : 0.32;
        ...
        if (this.nodes.whistleOnly)
            this.nodes.whistleOnly.checked = state.config.whistleOnly;
        ...
        if (this.nodes.wakeLock)
            this.nodes.wakeLock.checked = state.config.wakeLock;
        this.nodes.longExposure.checked = state.config.longExposure;
        this.nodes.scaleFactor.value = parseFloat(
            state.config.scaleFactor,
        ).toFixed(2);
        ...
        if (state.openHelpTopic) {
            this.nodes.helpModalHeader.textContent = this.i18n.t(
                `help.${state.openHelpTopic}.h`,
            );
            this.nodes.helpModalBody.textContent = this.i18n.t(
                `help.${state.openHelpTopic}.b`,
            );
        }
    }
}
