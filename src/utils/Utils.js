...
export class Utils {
    ...
    static makePistilColor(shellColor) {
        return shellColor === Constants.COLOR.White ||
            shellColor === Constants.COLOR.Gold
            ? Utils.randomColor({
                  notColor: shellColor,
              })
            : Utils.whiteOrGold();
    ...
    static createParticleArc(
        ...
        if (end > start) {
            for (let angle = start; angle < end; angle = angle + angleDelta)
                particleFactory(
                    angle + Math.random() * angleDelta * randomness,
                );
        } else {
            for (let angle = start; angle > end; angle = angle + angleDelta)
                particleFactory(
        ...
        for (let i = 0; i <= C_HALF; i++) {
            ...
            for (let j = 0; j < partsPerArc; j++)
                particleFactory(
                    angleInc * j +
                        angleOffset +
                        Math.random() * maxRandomAngleOffset,
                    ringSize,
                );
        }
    }
